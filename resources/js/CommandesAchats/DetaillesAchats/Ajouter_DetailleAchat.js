let selectedArticles = new Set();
    ...
    if (!prix || isNaN(prix) || prix <= 0 || !quantite || isNaN(quantite) || quantite <= 0) {
        alert('Veuillez entrer un prix unitaire valide (> 0) et une quantité valide (> 0).');
        return;
    }
    ...
    if (selectedArticles.has(articleId)) {
        ...
        let index = document.getElementById("Liste_Articles").rows.length - 2; // -1 pour la ligne TH, -1 car l'index commence à 0.
