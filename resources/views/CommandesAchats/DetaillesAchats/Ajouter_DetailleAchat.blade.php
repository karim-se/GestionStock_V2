 ...
 @push('styles')
   ...
   <form  class="form" method="POST" action ="{{ route('CommandesAchats.DetaillesCommandeAchats.store',[$commandeAchat->CommandeAchatID]) }}" >
     @csrf
       ...
       @if (session('error'))
            <div class="error-message-box" style="color: red ; text-align:center;font-weight: bold;">
                {{ session('error') }}
            </div>
        @endif
    ...
    <div class="ligne-article">
        ...
        <div class="champ bouton">
            <button type="button" onclick="Afficher_Articles()" class="bg-gray-800 text-white p-2 rounded">Ajouter Ligne</button>
        </div>
